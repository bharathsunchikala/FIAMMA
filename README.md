...
You can run the sample locally in one of two ways:

1. Start the projects directly. This is the simplest option if you just want the site running locally.
   - From `src/PublicApi`, run `dotnet run`
   - From `src/Web`, run `dotnet run --launch-profile https`
   - Browse to `https://localhost:5001/`
   - The admin UI is available at `https://localhost:5001/admin`

2. Start the Aspire app host from `src/Fiamma.AppHost` with `dotnet run`.
   - The Aspire dashboard runs on `https://localhost:17177/` by default
   - The dashboard shows the current endpoints for the `web` and `publicapi` projects
   - This path also starts a Seq container for observability, so having Docker available is recommended

Most of the site's functionality works with just the web application running. However, the site's Admin page relies on Blazor WebAssembly running in the browser, and it must communicate with the server using the site's PublicApi web application, so you'll need both `Web` and `PublicApi` running for `/admin`.
